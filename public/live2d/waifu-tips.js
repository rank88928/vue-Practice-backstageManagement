...
!(function () {
  ...
  class s {
    ...
    async loadModel(t, s, n) {
      ...
      } else loadlive2d('live2d', `${this.apiPath}get/?id=${t}-${s}`), console.log(`Live2D 模型 ${t}-${s} 加載完成`);
    }
    async loadRandModel() {
      ...
      if (this.useCDN) {
        ...
        fetch(`${this.apiPath}rand_textures/?id=${t}-${s}`)
          .then((e) => e.json())
          .then((e) => {
            1 !== e.textures.id || (1 !== s && 0 !== s) ? this.loadModel(t, e.textures.id, '我的新衣服好看嘛？') : o('我還没有其他衣服呢！', 4e3, 10);
          });
    }
    ...
    hitokoto: {
      ...
      callback: function () {
        fetch('https://v1.hitokoto.cn')
          .then((e) => e.json())
          .then((e) => {
            const t = `這句一言來自 <span>「${e.from}」</span>，是 <span>${e.creator}</span> 在 hitokoto.cn 投稿的。`;
            o(e.hitokoto, 6e3, 9),
              setTimeout(() => {
                o(t, 4e3, 9);
              }, 6e3);
      ...
      window.addEventListener('mousemove', () => (n = !0)),
        ...
        o(
          (function (e) {
            ...
            const t = `歡迎使用<span>「${document.title.split(' - ')[0]}」</span>`;
            let o;
